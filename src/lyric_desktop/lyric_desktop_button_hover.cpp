...
// 设置提示信息
void lyric_wnd_set_tips(LYRIC_DESKTOP_INFO& wnd_info, LPCWSTR pszTips);

// 隐藏提示信息
void lyric_wnd_hide_tips(LYRIC_DESKTOP_INFO& wnd_info);
...
void lyric_wnd_button_hover(LYRIC_DESKTOP_INFO& wnd_info)
{
    ...
    case LYRIC_DESKTOP_BUTTON_ID_AHEAD:         // 歌词提前
    {
        ...
        {
            // 值是0, 表示没有延时歌词
            pszTips = id == LYRIC_DESKTOP_BUTTON_ID_BEHIND
                ? L"歌词延后0.5秒, 当前没有延后"
                : L"歌词提前0.5秒, 当前没有提前";
        }
        else if (wnd_info.nTimeOffset > 0)
        {
            // 延后是负数, 提前是正数
            // 大于0, 那就是歌词提前了
            LPCWSTR fmt = id == LYRIC_DESKTOP_BUTTON_ID_BEHIND
                ? L"歌词延后, 当前提前了 %.1f 秒"
                : L"歌词提前, 当前提前了 %.1f 秒";
